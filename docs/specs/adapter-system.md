...
1. **Hosts read the same intent from different _locations_.** A skill's agent-vs-user invocation policy lives in `SKILL.md` frontmatter on Claude/Cursor (`disable-model-invocation`) but in a **sidecar `agents/openai.yaml`** (`policy.allow_implicit_invocation`, inverted) on Codex. Hooks live in `hooks/claude.json` on Claude, `hooks/hooks.json` on Codex. Author-once means the toolkit must **emit each host's native sidecar from the shared source** — not omit the capability.
...
**Explicitly _not_ a driver (corrected from 0.1.0):** per-host `SKILL.md` frontmatter rewriting. Claude, Cursor, and Codex are all **lenient at runtime** — they require only `name`+`description` and tolerate unknown keys; the only load-fatal frontmatter defect is **invalid YAML**, which is a _validate-time gate on the source_ (`frontmatter-invalid`, already shipped in core 0.5.0), not an adaptation. (Evidence: `liaison` failed to load on Codex purely on a YAML parse error — an unquoted `": "` — not on any key; `dream` loaded fine on Codex carrying `arguments`/`user-invocable`.)
...
- **Sidecar emission** (source-installed hosts): generate committed per-host files _in the source plugin dir_ (hook JSON, `agents/openai.yaml`). The shared `SKILL.md` is **not** rewritten.
...
- **Not a quality evaluator.** Whether an adapted skill _works well_ on a host is a future eval harness (`architecture.md` §8.3), out of scope.
- **Not per-host frontmatter rewriting** for currently-targeted hosts — reserved (§4.4) for a future _runtime-strict_ host, none of which we target.

---

## 2. Governing decisions (reconciling P2 and P4)

> **D1 (normative).** Adaptation is permitted **only** as a **source→target** projection of _shared authored content_, and **only** when deterministic and rule-driven (§2.2). **Target→target** translation stays forbidden (P4 unchanged). Adapters are `N` projections from one shared source — linear in targets, each blind to the others — never an `N×N` matrix.

> **D2 (normative).** P2 is relaxed _for shared authored content only_. The envelope (§6) still governs _which_ targets emit; the adapter governs _how_ the shared content is shaped/located for each. No inference of one target's native manifest from another's.
...
§7.1 defines _mechanical_ as a pure function driven by a committed lookup table, bounded to one target. Adapters meet this, widened so an adapter MAY **drop, relocate, or merge** content **iff** the rule is committed data and the change is reported as a `Finding` (§5). The committed output keeps P5's "honest diffs" — the bytes plus the reason are both reviewable.
...
- **Source-installed** — the target's registry points at the source plugin dir (`./plugins/<name>`); the host reads authored files plus any committed **per-host sidecars** the toolkit emits there (e.g. `hooks/codex.json`, `agents/openai.yaml`). _Today and going forward:_ **`claude`, `cursor`, `codex`** — all lenient on the shared `SKILL.md`, all reading host-specific extras from sidecars.
- **Emitted-installed** — the registry/root artifact points at a toolkit-emitted `dist/<target>/` copy. _Today:_ **`gemini`, `kiro`**.

### 3.2 When a host must be emitted-installed

> **D3 (normative, revised).** A host MUST be emitted-installed **only** when it needs a _structural restructure of the plugin set_ that cannot be expressed as committed sidecars in the source tree — i.e. the **single-artifact hosts** (Gemini/Kiro: one repo-root artifact; the unifier merges N→1). A host that is lenient on the shared files and consumes host-specific extras via sidecars **stays source-installed**.
...
> **Why this is a "located", not a "rewritten", adaptation.** The source `SKILL.md` is not edited for Codex (Codex tolerates the extra keys). The adapter _adds_ `agents/openai.yaml`. Same shape as hook sidecars.
...
#### 4.2.1 The hook _contract_ is portable for Claude→Codex, divergent for Cursor (primary-verified)
...
| Stdin envelope    | `session_id`, `transcript_path`, `cwd`, `hook_event_name`, `tool_name`, `tool_input`, `tool_output`                    | **same** + `turn_id`/`model`; `tool_response` for output | **both** `conversation_id` **and** `session_id` (verified — see callout), `generation_id`, `workspace_roots`, `tool_name:"Shell"` (not `"Bash"`); shell events also expose `command`/`output` |
...
> **Empirical verification (2026-07-14, local CLIs — `cursor-agent` 2026.03.30, `codex exec` 0.144.4; headless, `preToolUse`/`PreToolUse` on a shell tool).** Captured the live stdin envelopes and probed the failure modes:
>
> - **Codex confirms D6a.** Real stdin: `hook_event_name:"PreToolUse"` (PascalCase), `session_id`, `tool_name:"Bash"`, `tool_input:{command}` (+ additive `turn_id`/`model`/`permission_mode`). A Claude handler keyed on `tool_name==="Bash"` reading `session_id`/`tool_input.command` runs **unchanged** — identity-passthrough holds.
> - **Cursor confirms D6b's need, corrects two details.** Real stdin carries **both** `conversation_id` and `session_id` (same value) and `tool_name:"Shell"` (not `"Bash"`) — so a Claude `Bash`-keyed handler still misfires, but the shim maps `tool_name` rather than synthesizing a missing `session_id`. `{"permission":"deny"}` from `preToolUse` **blocks** (flat control field verified).
> - **Fail-open is narrower than stated.** For `preToolUse` in the CLI, **malformed JSON output _blocked_** the tool (fail-_closed_) — it did **not** silently allow (that forum report was for `beforeShellExecution`). The reproduced fail-open vector is a **non-zero handler exit** (handler crash → tool ran); adding **`failClosed:true`** converted it back to a block. So the shim's load-bearing mandate is "`failClosed:true` + emit `{"permission":"deny"}` and exit 2 on any handler error," more than "always emit valid JSON." Cloud-agent behavior (T6) remains unverified.

> **Governing distinction — tier by hook _intent_, not by host.**
>
...
> **D6b (normative) — Cursor controller hooks are served by a generated translation shim.** Because the Cursor handler contract diverges on every axis, the adapter wraps each Claude **controller** hook in a **shim command** that (1) reads Cursor stdin and rewrites it to the Claude envelope — the primary fix is mapping `tool_name:"Shell"`→`"Bash"` and the camelCase→PascalCase event name; `session_id` and `tool_input.command` are already present (verified — see §4.2.1 callout) and pass through, with `conversation_id` a fallback only if `session_id` is ever absent; (2) invokes the original Claude handler; (3) translates the Claude control output back to Cursor's (`permissionDecision`→`permission`, `decision:"block"`→`permission:"deny"`/`continue:false`, `updatedInput`→`updated_input`, `additionalContext`→`additional_context`). **Three non-negotiable safety mandates** — Cursor fails open on a non-zero handler exit by default (verified 2026-07-14 for `preToolUse` in the CLI; malformed JSON _blocked_ there, but [is reported to silently allow for `beforeShellExecution`](https://forum.cursor.com/t/beforeshellexecution-hook-malformed-json-response-silently-allows-command-instead-of-blocking/152669), so treat malformed-output behavior as event-dependent and unsafe to rely on): the shim's Cursor hook entry MUST set **`failClosed: true`** (empirically converts a crashing handler back to a block); the shim MUST **always emit syntactically valid JSON**; and on any error / missing field / non-zero handler exit the shim MUST emit **`{"permission":"deny"}` and exit `2`** (fail-safe, inverting Cursor's default). Soft `hook-controller-shimmed-cursor`. **Observer** hooks take the event-rename path with no shim.
...
> **D7 (normative).** **Skill-frontmatter hooks** (Claude's `hooks:` block inside a `SKILL.md`, skill-_scoped_) are an **opt-in** authoring path, not the default:
>
> - **Claude → passthrough** (native skill-scoping preserved).
> - **Codex/Cursor → promote** to the plugin hook artifact (no skill-scoping on those hosts) and emit a soft **`hook-scope-widened`** finding: _"skill-scoped hook `<name>` becomes plugin-wide (always-on) on `<target>`; if it is a blocking/deny gate it will fire outside this skill — confirm intended."_
>
...
Fills the seam `dogfood-marketplace-and-native-install.md` reserves (_"Merging N→1 … a future `aggregate` opt-in"_). For single-artifact hosts there are **two** paths:
...
### 4.4 Frontmatter _rewrite_ adapter — RESERVED [Future, seam reserved]
...
- **Verify-tasks (must resolve before the relevant code ships):**
  ...
  - **T6** — Whether Cursor honors **`failClosed: true` in cloud agents** (the §4.2.1 D6b shim's fail-safe depends on it). **CLI verified 2026-07-14** (`failClosed:true` converts a crashing `preToolUse` handler to a block). **Decision 2026-07-14:** the cloud case is not empirically testable from the local CLI, so the controller-shim work proceeds on the assumption **cloud == local** (`cursor-controller-shim.md` §2.1); the shim's fail-safe never relies on the fail-open default, so a future refutation only affects the `failClosed` entry flag.
  - **T7** — **Shell-event ↔ `Bash`-tool stdin synthesis fidelity**: confirm a Claude handler keyed on `tool_name:"Bash"` / `tool_input.command` behaves correctly when fed from Cursor's shell payload. **Partially resolved 2026-07-14** — verified `preToolUse` on a shell tool reports `tool_name:"Shell"` with `tool_input.command` present, so the shim must map `tool_name`→`"Bash"` (the command field carries over directly); the `beforeShellExecution` `command`/`cwd`/`sandbox` shape and `afterShellExecution` `output`→`tool_output` mapping are still to confirm.
