...
- **Verify-tasks (must resolve before the relevant code ships):**
  ...
  - **T6** — Whether Cursor honors **`failClosed: true` in cloud agents** (the §4.2.1 D6b shim's fail-safe depends on it). **CLI verified 2026-07-14** (`failClosed:true` converts a crashing `preToolUse` handler to a block); the **cloud** case remains open.
