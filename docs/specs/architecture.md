...
**Status:** Draft
**Spec version:** 0.4.0
**Supersedes:** 0.3.0 (pre-1.0 API tightening; review polish; two future-breaking type risks closed)
...
> **What changed from 0.3.0 → 0.4.0.** Targeted polish after a third review pass. Two future-breaking API risks closed: `Finding.code` is now a typed, additive union (`FindingCode`) rather than an open `string`; `GeneratedFile.target` drops the unused `'shared'` variant. The `defineConfig` brand contract is pinned (`AipmConfigInput` → branded `AipmConfig`). Added concrete reference file layout for per-target modules, named the cross-target-import CI tool (eslint `no-restricted-imports` or dependency-cruiser), added sidecar-sentinel option for hosts with strict manifest schemas, clarified the public-subpath policy, and added a migrate no-op caveat for future compatibility. Cut §13.1 "what must not happen" (restated §12 and P7/P8) and a redundant sentence in §1.3.
...
The freshness check (§10.5) rejects any generated file whose content doesn't match what `aipm build` would produce, whether because the source changed or because the output was hand-edited.
...
export interface GeneratedFile {
  ...
  target: TargetId; // which target's build step produced it
}
