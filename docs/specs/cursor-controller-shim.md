...
**UAT (`cursor/cursor-shim.uat.test.ts`, new, gated on the CLI being present):** the real end-to-end proof. Build a temp workspace with a generated `hooks/cursor.json` + `hooks/cursor-shim.mjs` wrapping a Claude-format deny handler, run the Cursor CLI headless (`cursor-agent -p --trust --force --workspace <dir>` prompting a **benign** shell command that redirects output into an innocuously-named file), and assert the command is **blocked** — use the file's absence as ground truth (absent = blocked), not stdout scraping. The prompt is deliberately benign and the filename deliberately innocuous rather than a `touch MARKER_SHOULD_NOT_EXIST`-style instruction-following trap: current `cursor-agent` models refuse a trap-shaped prompt outright (never calling the Shell tool), which starves the gate of a call to intercept — a harness artifact, not a hook-invocation defect (see issue #83's determination). Skip with a logged notice when `cursor-agent` is not on `PATH` (a locally-automatable-but-not-CI test). A second, narrower precondition applies once the CLI is present: the positive control (a `HOOK_FIRED` marker the wrapped handler writes, proving the generated `preToolUse` hook actually ran) must hold before the enforcement assertion runs — if the installed `cursor-agent` build still never invokes the hook for the benign prompt (e.g. a genuinely old/incompatible CLI build), the suite self-skips with a logged reason at that precondition instead of hard-failing; it does **not** soften the enforcement assertion itself (marker absence), which still runs and still fails loudly once the hook is proven to have fired. On a current, compatible `cursor-agent` build this UAT runs to completion and asserts real enforcement — it does not skip. This closes the loop the unit tests can't: that a real Claude deny gate now enforces on real Cursor.
