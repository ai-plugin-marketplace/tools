...
**UAT (`cursor/cursor-shim.uat.test.ts`, new, gated on the CLI being present):** the real end-to-end proof. Build a temp workspace with a generated `hooks/cursor.json` + `hooks/cursor-shim.mjs` wrapping a Claude-format deny handler, run the Cursor CLI headless (`cursor-agent -p --trust --force --workspace <dir>` prompting a shell command), and assert the command is **blocked** — use a `touch <marker>` side-effect as ground truth (marker absent = blocked), not stdout scraping. Skip with a logged notice when `cursor-agent` is not on `PATH` (a locally-automatable-but-not-CI test). A second, narrower precondition applies once the CLI is present: the positive control (a `HOOK_FIRED` marker the wrapped handler writes, proving the generated `preToolUse` hook actually ran) must hold before the enforcement assertion runs — if the installed `cursor-agent` build never invokes the hook (e.g. CLI version drift), the suite self-skips with a logged reason at that precondition instead of hard-failing; it does **not** soften the enforcement assertion itself (marker absence), which still runs and still fails loudly once the hook is proven to have fired. This closes the loop the unit tests can't: that a real Claude deny gate now enforces on real Cursor.
