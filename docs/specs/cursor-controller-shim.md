...
Verified against the local CLIs (`cursor-agent` 2026.03.30, `codex exec` 0.144.4) — see
`adapter-system.md` §4.2.1 callout and the solutions note
`verify-agent-hook-contracts-with-local-clis.md`:
...
- If any gating-event hook is present, additionally emit `hooks/cursor-shim.mjs` (static content +
  `_generated` sentinel; JS uses a line-comment sentinel, not a JSON field — see §3.4).
- Freshness compares both `hooks/cursor.json` and `hooks/cursor-shim.mjs` byte-for-byte.

### 3.4 Sentinel

`hooks/cursor.json` keeps its JSON-field sentinel. `hooks/cursor-shim.mjs` carries a **leading
line-comment sentinel** (`// @generated by @ai-plugin-marketplace/core — do not edit; source: hooks/claude.yaml`)
— the JS analogue of the JSON `_generated` field, following the existing sentinel conventions
(architecture §4.3). Freshness strips/ignores nothing for JS — the whole file including the comment is
...
**Build (`build.test.ts`, extend):** cursor envelope + a gating-event source → emits both `hooks/cursor.json` (shimmed entries) and `hooks/cursor-shim.mjs` (sentinel); observer-only source → no shim file; freshness round-trips both byte-for-byte.

**UAT (`cursor/cursor-shim.uat.test.ts`, new, gated on the CLI being present):** the real end-to-end proof, using the harness from `verify-agent-hook-contracts-with-local-clis.md`. Build a temp workspace with a generated `hooks/cursor.json` + `hooks/cursor-shim.mjs` wrapping a Claude-format deny handler, run `cursor-agent -p --trust --force`, and assert the shell command is **blocked** (side-effect file absent). Skip with a logged notice when `cursor-agent` is absent (per manual-test-design). This closes the loop the unit tests can't: that a real Claude deny gate now enforces on real Cursor.
