...
| Plugin version                                                                              | `aipm.config.ts` `version`                                                            | already exists (spec §9); today re-typed into manifests; drift is caught by `version-consistency` (`aipm validate`) pending this spec's codegen |
